// js/script.js (client-side) — использует GAS proxy для всех запросов к Supabase
// ЗАМЕНИТЕ:
const GAS_URL = 'https://script.google.com/macros/s/AKfycbyd_957oCLoHW7edb_QycsEy2KoykLRW_RZMYS4fMY5-9o7-ewYf_tU6Z3TCC1ugVbpww/exec'; // <- вставьте сюда web app URL из GAS
...
// DOM элементы найдём при DOMContentLoaded
...
// ---- loadAllData (использует GAS для Supabase-операций) ----
...
// ---- Текущие статистики: Fetch Roblox напрямую, а запись — через GAS ----
  ...
  try {
    ...
    if (res && res.ok) {
      ...
      if (game) {
        ...
        // сохраняем в Supabase через GAS
        ...
        }}).catch(e => console.warn('insert_player_stats error', e));

        // обновляем/вставляем рекорд дня через GAS (upsert)
        ...
        apiPost('upsert_daily_record', { record: {
          ...
          value: game.playing,
          recorded_at: today
        }}).catch(e => console.warn('upsert_daily_record error', e));
    ...
    console.error('loadCurrentStats', err);
  }
}

// ---- calculateFavoritesGrowth (через GAS: get_player_stats_since) ----
async function calculateFavoritesGrowth(elements, currentFavorites) {
  ...
  try {
    ...
    console.error('calculateFavoritesGrowth', err);
  }
}
...
function createCharts(labels, data) {
  ...
  charts.main = new Chart(ctx, {
    ...
    data: { labels, datasets: [{ label: currentLang === 'en' ? 'Players' : 'Игроки', data, borderColor: '#00A2FF', backgroundColor: currentChartType === 'bar' ? 'rgba(0,162,255,0.25)' : 'rgba(0,162,255,0.08)', tension: 0.4, fill: currentChartType === 'line' }]},
...
// ---- Peak hours (через GAS get_hourly) ----
  ...
  try {
    ...
    if (Array.isArray(data) && data.length) {
      elements.peakHours.innerHTML = data.map((item, index) => {
        const hour = item.hour || item.hour_of_day || null;
        let timeLabel = '-';
        if (hour) {
          try {
            const d = isNaN(Date.parse(hour)) ? (() => { const t = new Date(); t.setHours(Number(hour), 0, 0, 0); return t; })() : new Date(hour);
        ...
        return `<div class="flex items-center justify-between p-3 bg-gray-200 dark:bg-gray-800 rounded-lg">
                  <div class="flex items-center gap-3"><span class="text-lg font-bold text-roblox-blue">#${index+1}</span><span>${timeLabel}</span></div>
